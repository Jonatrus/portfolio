...
then we obtain the Fibonacci sequence having terms Fn that are defined by the recurrence relation Fn=Fn−1+Fn−2
...
n = 33
k = 4
pop_seq = [0,1,1] # 0 is a placeholder to effectivaly make the list 1 based indexing.

# Loop range is is that the indexing matches the mathematical formula more closely.
for i in range(3,n+1):
    pop = pop_seq[i-1] + (k * pop_seq[i-2])
    pop_seq.append(pop)
